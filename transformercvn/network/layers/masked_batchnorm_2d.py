...
class MaskedBatchNorm2D(nn.Module):
    ...
    def forward(self, images: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        if C != self.num_features:
            raise ValueError(
                "Expected %d channels but images has %d channels" % (self.num_features, C)
            )

        if mask is not None:
            masked_images = images * mask.view(B, 1, 1, 1)
            n = mask.sum() * W * H
        else:
            masked_images = images
            n = torch.scalar_tensor(B * W * H, dtype=torch.int64)

        # Find the masked sum of the images
        masked_sum = masked_images.sum(dim=(0, 2, 3), keepdim=True)

        # Compute masked image statistics
        current_mean = masked_sum / n
        current_var = ((masked_images - current_mean) ** 2).sum(dim=(0, 2, 3), keepdim=True) / n
        ...
        if self.track_running_stats and not self.training:
            normed_images = (masked_images - self.running_mean) / (
